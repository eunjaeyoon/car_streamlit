...
def run_ml_app() :
    st.subheader('Machine Learning 책정')
    ...
    btn = st.button('책정 결과 보기')
    # 결과가 소수점으로 나오는데, 소수점 뒤 한자리까지만 나오도록
    # 코드 수정하세요.
    if btn :
        st.write('책정 결과! {:.1f} 달러의 차를 살 수 있습니다.'.format(y_pred[0,0]))
        st.write('책정 결과! {} 달러의 차를 살 수 있습니다.'.format( round(y_pred[0,0], 1) ))
